...
class EventCampaignViewSet(viewsets.ViewSet):
    queryset = Campaign.objects.all()
    serializer_class = CampaignSerializer
    ...
    def update(self, request, pk=None, event_pk=None):
        ...
        if cpg not in event.get_sub_entities_of_type(BaseEntityComponent.SUB_ENTITY_CAMPAIGN):
            return Response("event id %s not associated with Campaign %s " % (pk, event_pk),
                            status=status.HTTP_400_BAD_REQUEST)

        serializer = CampaignSerializer(cpg, data=request.data)
        if serializer.is_valid():
            inst = serializer.save()
...
class EventSpeakerViewSet(viewsets.ViewSet):
    queryset = Speaker.objects.all()
    ...
    def update(self, request, pk=None, event_pk=None):
        ...
        serializer = SpeakerSerializer(spk, data=request.data)
        if serializer.is_valid():
            inst = serializer.save()
            return Response(serializer.data)
    ...
    def partial_update(self, request, pk=None, event_pk=None):
        ...
        if spk not in event.get_sub_entities_of_type(BaseEntityComponent.SUB_ENTITY_SPEAKER):
            return Response("event id %s not associated with Speaker %s " % (pk, event_pk),
                            status=status.HTTP_400_BAD_REQUEST)
...
class EventSponsorViewSet(viewsets.ViewSet):
    queryset = Sponsor.objects.all()
    serializer_class = SpeakerSerializer
    ...
    def update(self, request, pk=None, event_pk=None):
        ...
        if spn not in event.get_sub_entities_of_type(BaseEntityComponent.SUB_ENTITY_SPONSOR):
            return Response("event id %s not associated with Sponsor %s " % (pk, event_pk),
                            status=status.HTTP_400_BAD_REQUEST)

        serializer = SponsorSerializer(spn, data=request.data)
        if serializer.is_valid():
            inst = serializer.save()
            return Response(serializer.data)
...
class EventMediaViewSet(viewsets.ViewSet):
    ...
    def update(self, request, pk=None, event_pk=None):
        ...
        if med not in event.get_sub_entities_of_type(BaseEntityComponent.SUB_ENTITY_MEDIA):
            return Response("event id %s not associated with Media %s " % (pk, event_pk),
                            status=status.HTTP_400_BAD_REQUEST)

        serializer = MediaEntitiesSerializer(med, data=request.data)
        if serializer.is_valid():
            inst = serializer.save()
            return Response(serializer.data)
...
class EventAttendeeViewSet(viewsets.ViewSet):
    queryset = AttendeeInvitee.objects.all()
    serializer_class = AttendeeInviteeSerializer
    ...
    def update(self, request, pk=None, event_pk=None):
        ...
        if ati not in event.get_sub_entities_of_type(BaseEntityComponent.SUB_ENTITY_ATTENDEE_INVITEE):
            return Response("event id %s not associated with Invitee %s " % (pk, event_pk),
                            status=status.HTTP_400_BAD_REQUEST)

        serializer = AttendeeInviteeSerializer(ati, data=request.data)
        if serializer.is_valid():
            inst = serializer.save()
            return Response(serializer.data)
...
class EventCoOwnerViewSet(viewsets.ViewSet):
    queryset = CoOwners.objects.all()
    serializer_class = CoOwnersSerializer
    ...
    def update(self, request, pk=None, event_pk=None):
        ...
        if coo not in event.get_sub_entities_of_type(BaseEntityComponent.SUB_ENTITY_COOWNER):
            return Response("event id %s not associated with CoOwner %s " % (pk, event_pk),
                            status=status.HTTP_400_BAD_REQUEST)

        serializer = CoOwnersSerializer(coo, data=request.data)
        if serializer.is_valid():
            inst = serializer.save()
            return Response(serializer.data)
...
class EventAgendaViewSet(viewsets.ViewSet):
    queryset = Agenda.objects.all()
    serializer_class = AgendaSerializer
...
class EventTagonomyViewSet(viewsets.ViewSet):
    ...
    def update(self, request, pk=None, event_pk=None):
        ...
        if cat not in event.get_sub_entities_of_type(BaseEntityComponent.SUB_ENTITY_CATEGORY):
            return Response("event id %s not associated with category %s " % (pk, event_pk),
                            status=status.HTTP_400_BAD_REQUEST)

        serializer = TaganomySerializer(cat, data=request.data)
        if serializer.is_valid():
            inst = serializer.save()
            return Response(serializer.data)
