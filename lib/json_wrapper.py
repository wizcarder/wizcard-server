...
class DataDumper:
    ...
    def dump(self, data,format='xml', response={}):
        """
        ...
        if(format == 'xml'):
            return pyxslt.serialize.toString(prettyPrintXml=False,data=self.output,)
        else:
             return json.dumps(self.output, cls=DateTimeAwareJSONEncoder)
