...
class Node(object):
    ...
    def subtree(self):
        return self.__repr__(child for child in self.children.itervalues())

    def dfs(self, prefix, out):
        append = prefix.append
        if self.value is not NULL:
             out.append((self.KeyFactory(prefix), self.value))
        subtree = self.children
        for part in subtree:
            append(part)
            next = subtree.get(part)
            next.dfs(prefix, out)
            del prefix[-1]
...
class Trie(DictMixin, object):
    ...
    def longest_common_prefix_value(self, key, default=NULL):
    ...
    def longest_prefix_item(self, key, default=NULL):
        ...
        if node.value is not NULL:
            ...
            return self.items(self.KeyFactory(prefix)), plen
